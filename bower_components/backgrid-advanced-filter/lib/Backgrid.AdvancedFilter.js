	...
	Backgrid.Extension.AdvancedFilter.FilterStateModel = Backbone.Model.extend({
	  ...
	  getFilterableColumns: function() {
	    ...
	    _.each(filterAbleColumns, function(col) {
            var sortValue = col.get("sortValue");
            var labelVal = col.get("nesting");
            if (labelVal !== undefined) {
                labelVal = labelVal.join(' | ') + ' | ' + col.get("label");
			} else {
                labelVal = col.get("label");
			}

	      result[sortValue !== undefined ? sortValue : col.get("name")] = {
	        label: labelVal,
